...
export class iCloudEndpoint implements Endpoint {
	...
	public get accountInfo(): iCloudService.AccountInfo | undefined {
	...
	/**
	 * Lists photos from a specific album with optional incremental filtering.
	 * Only returns photos modified after the given timestamp (T009).
	 *
	 * @param albumId - The album ID or name to list photos from
	...
	async listPhotos(
		...
		if (!album) {
			this.logger.warn({ albumId }, 'Album not found');
			return [];
		}
		...
		if (lastSyncTimestamp) {
			...
			this.logger.info(
				...
				'Filtered photos by last sync timestamp',
			);
	...
	/**
	 * Gets photo count for an album (helper for state tracking).
	 *
	 * @param albumId - The album ID or name
	 * @returns Number of photos in the album
	 */
	async getPhotoCount(albumId: string): Promise<number> {
		const photos = await this.listPhotos(albumId);
