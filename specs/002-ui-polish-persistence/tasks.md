...
- [ ] T001 Install dependencies: prisma, @prisma/client, zod, zod-express-middleware, @tanstack/react-query, react-hook-form, @hookform/resolvers, react-window in package.json
- [ ] T002 Create Prisma schema file at prisma/schema.prisma with all 6 models (Configuration, Album, PhotoRecord, SyncState, SyncHistory, DatabaseMetadata)
- [ ] T003 [P] Initialize Prisma client singleton in src/lib/prisma.ts with logging and graceful shutdown
- [ ] T004 [P] Create initial database migration with `prisma migrate dev --name init`
- [ ] T005 [P] Implement AES-256-GCM encryption utilities in src/lib/encryption.ts (encrypt, decrypt, generateKey functions)
- [ ] T006 [P] Create Zod validation middleware for Express in src/lib/validation.ts
- [ ] T007 Update .env.example with DATABASE_URL and ENCRYPTION_KEY placeholders
- [ ] T008 [P] Create docker-entrypoint.sh script to run migrations and JSON migration on startup
- [ ] T009 Update Dockerfile with multi-stage build: stage 1 for Prisma generation, stage 2 for runtime
- [ ] T010 Update docker-compose.yml to add volume mounts for ./data and ./logs directories
...
- [ ] T011 Create Zod schema for Configuration in src/schemas/configuration.schema.ts with all three sections (iCloud, Frame, Sync)
- [ ] T012 [P] Create Zod schema for PhotoRecord in src/schemas/photo.schema.ts with validation rules
- [ ] T013 [P] Create Zod schema for SyncState in src/schemas/sync.schema.ts with counter validation
- [ ] T014 Implement JSON-to-SQLite migration script in src/scripts/migrate-json-to-sqlite.ts
- [ ] T015 Add unit tests for encryption utilities in test/unit/encryption.test.ts
- [ ] T016 [P] Add unit tests for migration script in test/unit/migration.test.ts with fixture data
- [ ] T017 Update src/web-server.ts to initialize Prisma client and add error handling middleware

**Checkpoint**: Foundation ready - user story implementation can now begin in parallel

---

## Phase 3: User Story 1 - Streamlined Configuration Management (Priority: P1) 🎯 MVP
...
- [ ] T018 [P] [US1] Create ConfigurationService in src/services/ConfigurationService.ts with get/update/testICloud/testFrame methods
- [ ] T019 [P] [US1] Implement GET /api/configuration endpoint in src/web-server.ts using ConfigurationService
- [ ] T020 [P] [US1] Implement POST /api/configuration endpoint in src/web-server.ts with Zod validation
- [ ] T021 [P] [US1] Implement POST /api/configuration/test-icloud endpoint in src/web-server.ts
- [ ] T022 [P] [US1] Implement POST /api/configuration/test-frame endpoint in src/web-server.ts
- [ ] T023 [US1] Create ConfigurationForm component in web/src/components/ConfigurationForm.tsx with React Hook Form and zodResolver
- [ ] T024 [US1] Add three-section layout to ConfigurationForm: iCloudSection, FrameSection, SyncSection components
- [ ] T025 [US1] Update Configuration page in web/src/pages/Configuration.tsx to use ConfigurationForm component
- [ ] T026 [US1] Add connection test buttons and feedback UI to ConfigurationForm component
- [ ] T027 [US1] Update api.ts in web/src/services/api.ts to add configuration endpoints with TypeScript types
- [ ] T028 [US1] Integrate React Query for configuration state management in Configuration page
- [ ] T029 [US1] Add unit tests for ConfigurationService in test/unit/ConfigurationService.test.ts
- [ ] T030 [US1] Add integration tests for configuration API in test/integration/configuration-api.test.ts

**Checkpoint**: Configuration management fully functional - users can configure and test connections

---

## Phase 4: User Story 2 - Simplified Dashboard & Sync Control (Priority: P2)
...
- [ ] T031 [P] [US2] Create SyncStateService in src/services/SyncStateService.ts with EventEmitter for SSE broadcasting
- [ ] T032 [US2] Implement GET /api/sync/status endpoint in src/web-server.ts using SyncStateService
- [ ] T033 [P] [US2] Implement GET /api/sync/status/stream (SSE) endpoint in src/web-server.ts with EventSource protocol
- [ ] T034 [P] [US2] Implement POST /api/sync/start endpoint in src/web-server.ts with precondition checks
- [ ] T035 [P] [US2] Implement POST /api/sync/stop endpoint in src/web-server.ts with graceful shutdown
...
- [ ] T039 [US2] Update Dashboard page in web/src/pages/Dashboard.tsx to remove configuration widgets and use SyncStatusCard
- [ ] T040 [US2] Add Start/Stop buttons with state management to Dashboard page
- [ ] T041 [US2] Add link to Configuration page when no configuration exists in Dashboard
...
## Phase 6: User Story 4 - Reliable State Persistence with Database (Priority: P4)
...
- [ ] T057 [P] [US4] Add database transaction wrapper utilities to src/lib/prisma.ts for atomic operations
...
- **Test Tasks**: 10 tasks (unit + integration)

**MVP Path**: T001-T017 (Foundation) → T018-T030 (User Story 1) = 30 tasks

**All Stories Path**: Complete all 80 tasks for full feature implementation
